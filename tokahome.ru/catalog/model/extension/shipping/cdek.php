...
class ModelExtensionShippingCdek extends Model {
	...
	function getQuote($address)
	{
		...
		if ($to_data && $address['city']) {
			...
			foreach ($cities as $city) {
				$cdek_cities = $this->getURL($this->getInfo()->getAjaxUrl().'city/getListByTerm/json.php?q=' . urlencode($city) . '&name_startsWith=' . urlencode($city), new parser_json());
			...
			if (is_array($cdek_cities) && isset($cdek_cities['geonames']) && count($cdek_cities['geonames'])) {
				...
				if ($count = count($available)) {
					...
					if ($this->config->get('shipping_cdek_login') != '' && $this->config->get('shipping_cdek_password') != '') {
						$calc->setAuth($this->config->get('shipping_cdek_login'), $this->config->get('shipping_cdek_password'));
					}

					$cdek_default_size = $this->config->get('shipping_cdek_default_size');
						...
						if ($status) {
							...
							if ($status) {
								...
								if ($this->config->get('shipping_cdek_work_mode') == 'single') {
									...
									{
										if(!$set_postamat || (float)$mtfvalue['priceByCurrency'] < (float)$set_postamat['priceByCurrency'])
									...
									foreach ($results as $shipping_info) {
										...
										if (array_key_exists($shipping_info['tariffId'], $cdek_tariff_list)) {
											...
											if ($this->config->get('shipping_cdek_delivery_data')) {
												...
												if ((float)$this->config->get('shipping_cdek_more_days')) {
													foreach ($period as &$delivery_date) $delivery_date = date('Y.m.d', strtotime('+' . (int)$this->config->get('shipping_cdek_more_days') . ' day', strtotime(implode('.', array_reverse(explode('.', $delivery_date))))));
												}
											...
											if (in_array($tariff_info['mode_id'], array(2, 4, 5, 6))) {
	...
	private function getResult(CalculatePriceDeliveryCdek &$calc, $total = 0) {
		...
		if ($calc->calculate() === true) {
			...
			if (!$this->config->get('shipping_cdek_cache_on_delivery') || !array_key_exists('cashOnDelivery', $response['result']) || ($this->config->get('shipping_cdek_cache_on_delivery') && (float)$response['result']['cashOnDelivery'] && $total >= (float)$response['result']['cashOnDelivery'])) {
	...
	private function getPVZList($city_id, $weight = 0, $type) {
		...
		$pvz_list_data = $this->getURL($this->getInfo()->getBaseUrl().'pvzlist.php?cityid=' . $city_id.'&type=' . $type, new parser_xml());
		...
		if (isset($pvz_list_data->Pvz)) {
			...
			foreach ($pvz_list_data->Pvz as $pvz_info)
			{
				...
				/*if ($this->config->get('shipping_cdek_pvz_more_one') == 'first') {
					break;
				}*/
			}

			/*if ($this->config->get('shipping_cdek_pvz_more_one') == 'merge') {
