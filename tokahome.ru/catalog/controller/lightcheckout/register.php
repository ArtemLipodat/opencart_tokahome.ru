<?php
class ControllerLightcheckoutRegister extends Controller {
	public function index() {
		$this->load->language('lightcheckout/checkout');
		
		$language_get = array('text_your_details','entry_customer_group','entry_firstname','entry_lastname','entry_telephone','entry_email','text_your_password','entry_password','entry_confirm','entry_company','entry_address_1','entry_address_2','entry_city','entry_postcode','entry_country','entry_zone','text_none');
		...
		if (!isset($data['status']['payment_address'])) {
		...
		if ($this->customer->isLogged()) {
			...
			if (!isset($status['shipping']) and isset($this->session->data['payment_address'])) {
		...
		$data['fields'] = $this->model_extension_module_lightcheckout->getBaseFields('checked_alogin', 'alogged');
		
		$data['fields'] = $this->model_extension_module_lightcheckout->getBaseFields('checked_alogin', 'register');
		
		$data['fields_address'] = $this->model_extension_module_lightcheckout->getBaseFields('checked_payment_address', 'payment_address');
		
		$data['batches_address'] = $this->model_extension_module_lightcheckout->getBaseFields('checked_alogin', 'batches_address');
		
		$this->response->setOutput($this->load->view('lightcheckout/register', $data));
	}
	...
	public function save() {
		...
		if (!$this->customer->isLogged()) {		
			...
			if (!$json) {
				...
				$this->load->model('extension/module/lightcheckout');
				$fields = $this->model_extension_module_lightcheckout->getBaseFields('checked_alogin');
				$fields_address = $this->model_extension_module_lightcheckout->getBaseFields('checked_payment_address', 'payment_address');

				$settings = $this->model_extension_module_lightcheckout->getBaseSettings();
				
				/*
				$guest = '';
				if (isset($this->request->post['account']) and $this->request->post['account'] == 'guest') {
					$guest = 'guest_';
					$fields_address = $fields;
				}
				*/
				...
				if (isset($this->request->post['email'])) {
					if (utf8_strlen($this->request->post['email']) > 0 or (!$this->request->post['email'] and $fields['email']['required'] and $fields['email']['show'])) {
				...
				if (isset($this->request->post['company']) and  $fields_address['company']['required'] and $fields_address['company']['show']) {
				...
				if (isset($this->request->post['address_1']) and  $fields_address['address_1']['required'] and $fields_address['address_1']['show']) {
					if ((utf8_strlen(trim($this->request->post['address_1'])) < 3) || (utf8_strlen(trim($this->request->post['address_1'])) > 128)) {
						$json['error']['address_1'] = $this->language->get('error_address_1');
					}
				}
				if (isset($this->request->post['address_2']) and  $fields_address['address_2']['required'] and $fields_address['address_2']['show']) {
				...
				if (isset($this->request->post['city']) and  $fields_address['city']['required'] and $fields_address['city']['show']) {
				...
				if (isset($this->request->post['light_country_id'])) {
					...
					if ((($country_info && $country_info['postcode_required']) or ($fields_address['postcode']['required'] and $fields_address['postcode']['show'])) && (utf8_strlen(trim($this->request->post['postcode'])) < 2 || utf8_strlen(trim($this->request->post['postcode'])) > 10)) {
						$json['error']['postcode'] = $this->language->get('error_postcode');
					}
					if ($fields_address['country_id']['required'] and $fields_address['country_id']['show']) {
						if ($this->request->post['light_country_id'] == '') {
							$json['error']['country'] = $this->language->get('error_country');
						}
					}
				}
				if (isset($this->request->post['light_zone_id']) and  $fields_address['zone_id']['required'] and $fields_address['zone_id']['show']) {
					if ($this->request->post['light_zone_id'] == '' || !is_numeric($this->request->post['light_zone_id']) || !$this->request->post['light_zone_id']) {
				...
				if (!isset($this->request->post['light_zone_id'])){$this->request->post['light_zone_id'] = '';}
				if (!isset($this->request->post['light_country_id'])){$this->request->post['light_country_id'] = '';}
				...
				if (isset($fields_address['custom']) and $fields_address['custom']['show']) {
			...
			if (!$json) {
				...
				if (!isset($this->request->post['firstname']) or !$this->request->post['firstname']) {
					$this->request->post['firstname'] = '';
				}
				if (!isset($this->request->post['lastname']) or !$this->request->post['lastname']) {
					$this->request->post['lastname'] = '';
				}
				if (!isset($this->request->post['telephone']) or !$this->request->post['telephone']) {
					$this->request->post['telephone'] = '';
				}
				if (!isset($this->request->post['fax']) or !$this->request->post['fax']) {
				...
				if ($customer_group_info && !$customer_group_info['approval']) {
					...
					} elseif (!empty($this->request->post['shipping_address'])) {
